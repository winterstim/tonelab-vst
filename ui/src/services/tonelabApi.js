...
async function requestAiChain(prompt, accessToken, signal) {
    ...
    remoteLog(`[Assistant] Request URL: ${apiUrl}`);
    remoteLog(`[Assistant] Origin: ${typeof window !== 'undefined' ? window.location.origin : 'unknown'}`);
    let response = null;
    ...
    for (let attempt = 1; attempt <= NETWORK_MAX_ATTEMPTS; attempt += 1) {
        try {
            ...
            remoteLog(`[Assistant] Network error on attempt ${attempt}: ${error?.message || 'unknown error'}. Retrying...`);
    ...
    if (!response.ok) {
        ...
        if (response.status === 403) {
            ...
            throw new Error(message || 'Active Tonelab Plus subscription required. Redirecting to pricing...');
        }
        throw new Error(`Assistant endpoint error: ${message}`);
    }

    const payload = await parseJsonResponse(response);
    const normalized = normalizeChainPayload(payload);

    remoteLog(`[Assistant] Raw chain payload type: ${Array.isArray(payload) ? 'array' : typeof payload}`);
    remoteLog(`[Assistant] Normalized effects: ${normalized.chain.length}, issues: ${normalized.issues.length}`);

    if (normalized.issues.length > 0) {
        throw new Error(`Assistant returned invalid params format: ${normalized.issues[0]}`);
    }
...
export async function generateChainFromPrompt(prompt, { signal, onStatus } = {}) {
    ...
    onStatus?.('Sending request to Assistant...');
    try {
